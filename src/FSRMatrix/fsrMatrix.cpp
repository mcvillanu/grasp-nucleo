...
void FSRMatrix::initPins(){
    pinMode(PINS::PIN_ADC_INPUT, INPUT);
    pinMode(PINS::PIN_SHIFT_REGISTER_DATA, OUTPUT);
    pinMode(PINS::PIN_SHIFT_REGISTER_CLOCK, OUTPUT);
    pinMode(PINS::PIN_MUX_CHANNEL_0, OUTPUT);
    pinMode(PINS::PIN_MUX_CHANNEL_1, OUTPUT);
    pinMode(PINS::PIN_MUX_CHANNEL_2, OUTPUT);
    pinMode(PINS::PIN_MUX_INHIBIT_0, OUTPUT);
    pinMode(PINS::PIN_MUX_INHIBIT_1, OUTPUT);
    pinMode(PINS::PIN_ADC_INPUT, INPUT);
    pinMode(PINS::PIN_SHIFT_REGISTER_DATA, OUTPUT);
    pinMode(PINS::PIN_SHIFT_REGISTER_CLOCK, OUTPUT);
    pinMode(PINS::PIN_MUX_CHANNEL_0, OUTPUT);
...
void FSRMatrix::setRow(int rowNumber)
{
    ...
    {
        digitalWrite(PINS::PIN_MUX_INHIBIT_0 + current_enabled_mux, HIGH); //Muxes are enabled using offset from MUX_INHIBIT_0. This is why mux inhibits MUST be wired to consecutive Arduino pins!
        current_enabled_mux++;
        if (current_enabled_mux >= MUX_COUNT)
        {
            current_enabled_mux = 0;
        }
        digitalWrite(PINS::PIN_MUX_INHIBIT_0 + current_enabled_mux, LOW); //enable the next mux, active low
    }
    ...
    for (int i = 0; i < CHANNEL_PINS_PER_MUX; i++)
    {
        ...
        {
            digitalWrite(PINS::PIN_MUX_CHANNEL_0 + i, HIGH);
        }
        else
        {
            digitalWrite(PINS::PIN_MUX_CHANNEL_0 + i, LOW);
        }
    }
}
...
void FSRMatrix::shiftColumn(bool isFirst)
{
    if (isFirst)
    {
        digitalWrite(PINS::PIN_SHIFT_REGISTER_DATA, HIGH);
    }
    digitalWrite(PINS::PIN_SHIFT_REGISTER_CLOCK, HIGH);
    digitalWrite(PINS::PIN_SHIFT_REGISTER_CLOCK, LOW);
    if (isFirst)
    {
        digitalWrite(PINS::PIN_SHIFT_REGISTER_DATA, LOW);
    }
}
...
void FSRMatrix::read(){
    for (int i = 0; i < FSR_MATRIX_ROW_COUNT; i++)
    {
        ...
        for (int j = 0; j < FSR_MATRIX_COLUMN_COUNT; j++)
        {
            ...
            int rawReading = analogRead(PINS::PIN_ADC_INPUT);
